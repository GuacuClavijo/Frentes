...
function updateSolutions(){const types=[...$('tipo').selectedOptions].map(o=>o.value);const old=[...$('solucao').selectedOptions].map(o=>o.value);let arr=D.filter(r=>!types.length||types.includes(String(val(r,'TIPO')).trim())).map(r=>String(val(r,'SOLUÇÃO')).trim()).filter(x=>x&&x!=='-');arr=[...new Set(arr)].sort((a,b)=>a.localeCompare(b,'pt-BR'));fillSelect('solucao',arr);[...$('solucao').options].forEach(o=>o.selected=old.includes(o.value));}
function apply(){
...
function setupZoom(){
  ...
  axis.addEventListener('pointerdown',e=>{
    ...
    if(e.target.closest('button,.stationMarker'))return;
    drag={x:e.clientX,lo,hi,span:hi-lo}; axis.setPointerCapture?.(e.pointerId); axis.classList.add('dragging'); e.preventDefault();
  });
  axis.addEventListener('pointermove',e=>{
    if(!drag||e.pointerType!=='mouse')return;
    const rect=axis.getBoundingClientRect(),dx=e.clientX-drag.x,delta=-(dx/Math.max(rect.width,1))*drag.span;
    let nlo=drag.lo+delta,nhi=drag.hi+delta;
    if(nlo<MIN){nhi+=MIN-nlo;nlo=MIN} if(nhi>MAX){nlo-=nhi-MAX;nhi=MAX}
    lo=nlo;hi=nhi;$('r1').value=lo;$('r2').value=hi;apply(); e.preventDefault();
  });
  const endMouse=e=>{if(drag&&e.pointerType==='mouse'){drag=null;axis.classList.remove('dragging')}};
