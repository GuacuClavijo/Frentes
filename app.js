...
function apply(){
 ...
 filtered=D.filter(r=>Number.isFinite(+r._ki)&&Number.isFinite(+r._kf)&&+r._kf>=lo&&+r._ki<=hi&&(!tr||String(val(r,'TRECHO')).trim()===tr)&&(!tp||String(val(r,'TIPO')).trim()===tp)&&(!ld||String(val(r,'LADO')).trim()===ld)&&(!so||String(val(r,'SOLUÇÃO')).trim()===so)&&(!statusFilter||releaseStatus(r)===statusFilter));
