...
function setupZoom(){
  const axis=$('axis'); if(!axis)return;
  const zoomSurface=axis.closest('.axiswrap')||axis;
  const onWheel=e=>{
    if(e.ctrlKey)return;
    e.preventDefault();
  ...
  zoomSurface.addEventListener('wheel',onWheel,{passive:false});
  let drag=null,dragMoved=false,suppressClick=false;
  axis.addEventListener('pointerdown',e=>{
    if(e.pointerType!=='mouse'||e.button!==0)return;
    drag={x:e.clientX,y:e.clientY,lo,hi,span:hi-lo}; dragMoved=false;
    axis.setPointerCapture?.(e.pointerId); axis.classList.add('dragging'); e.preventDefault();
  });
  axis.addEventListener('pointermove',e=>{
    if(!drag||e.pointerType!=='mouse')return;
    const rect=axis.getBoundingClientRect(),dx=e.clientX-drag.x,dy=e.clientY-drag.y;
    if(Math.abs(dx)>4||Math.abs(dy)>4)dragMoved=true;
    const delta=-(dx/Math.max(rect.width,1))*drag.span;
    let nlo=drag.lo+delta,nhi=drag.hi+delta;
    if(nlo<MIN){nhi+=MIN-nlo;nlo=MIN} if(nhi>MAX){nlo-=nhi-MAX;nhi=MAX}
    if(dragMoved){lo=nlo;hi=nhi;$('r1').value=lo;$('r2').value=hi;apply();e.preventDefault();}
  });
  const endMouse=e=>{
    if(!drag||e.pointerType!=='mouse')return;
    suppressClick=dragMoved;
    if(dragMoved)setTimeout(()=>{suppressClick=false},80);
    drag=null;axis.classList.remove('dragging');
  };
  axis.addEventListener('pointerup',endMouse);axis.addEventListener('pointercancel',endMouse);
  axis.addEventListener('click',e=>{if(suppressClick){e.preventDefault();e.stopImmediatePropagation();suppressClick=false}},true);
  ...
  axis.addEventListener('pointerdown',e=>{
    ...
    if(pts.size===2){const [a,b]=[...pts.values()];pinch={distance:Math.max(1,distance(a,b)),midX:midpoint(a,b),lo,hi};axis.classList.add('pinching');}
  });
  axis.addEventListener('pointermove',e=>{
...
/* V25 REAL UI */
(function(){
 const defs=[['tipo','Tipo'],['solucao','Solução'],['lado','Lado']];
 function build(){
  const host=document.getElementById('filterLeftV25'); if(!host||host.dataset.ready)return;
  host.dataset.ready='1';
  defs.forEach(([id,label])=>{
   const src=document.getElementById(id); if(!src)return;
   const box=document.createElement('div'); box.className='v25Filter';
   box.innerHTML='<label>'+label+'</label><button type="button" class="v25Trigger"><span>Todos</span><b>⌄</b></button><div class="v25Panel"></div>';
   const panel=box.querySelector('.v25Panel'), trig=box.querySelector('.v25Trigger'), text=trig.querySelector('span');
   Array.from(src.options).forEach(o=>{const lab=document.createElement('label');lab.className='v25Option';const cb=document.createElement('input');cb.type='checkbox';cb.value=o.value;cb.checked=o.selected;lab.append(cb,document.createTextNode(o.textContent));panel.appendChild(lab);cb.addEventListener('change',()=>{Array.from(src.options).forEach(x=>x.selected=Array.from(panel.querySelectorAll('input:checked')).some(y=>y.value===x.value));text.textContent=Array.from(panel.querySelectorAll('input:checked')).map(x=>x.parentElement.textContent.trim()).join(', ')||'Todos';src.dispatchEvent(new Event('change',{bubbles:true}))})});
   trig.addEventListener('click',e=>{e.stopPropagation();document.querySelectorAll('.v25Filter.open').forEach(x=>x!==box&&x.classList.remove('open'));box.classList.toggle('open')});
   panel.addEventListener('click',e=>e.stopPropagation()); host.appendChild(box);
  });
  document.addEventListener('click',()=>document.querySelectorAll('.v25Filter.open').forEach(x=>x.classList.remove('open')),{passive:true});
 }
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',build);else build();
})();
/* V25 REAL crossing markers: ENCONTRO 1/2 remain only in the white axis band */
(function(){
 function addCrossings(){
  const axis=document.getElementById('axis'); if(!axis||!window.GERAL)return;
  axis.querySelectorAll('.crossingLineV25').forEach(x=>x.remove());
  const rows=window.GERAL||[]; const span=Math.max(hi-lo,.001);
  rows.forEach(r=>{const id=String((typeof val==='function'?val(r,'IDENTIFICAÇÃO'):r.IDENTIFICAÇÃO)||'').toUpperCase(); if(!/ENCONTRO\s*[12]/.test(id))return; const km=(+r._ki + +r._kf)/2/1000; if(km<lo||km>hi)return; const d=document.createElement('div');d.className='crossingLineV25';d.style.left=((km-lo)/span*100)+'%';d.title=id;d.innerHTML='<span>'+id.match(/ENCONTRO\s*[12]/)[0]+'</span>';axis.appendChild(d)});
 }
 const oldRender=window.render; if(typeof oldRender==='function'){}; setTimeout(addCrossings,1200); window.addEventListener('resize',()=>setTimeout(addCrossings,0));
})();
